...
export function createPackworksScene(canvas, getGameState) {
  ...
  function drawWorker(x, y, set, variant = 0) {
    const bob = Math.sin(elapsed * 0.004 + variant * 2) * 0.04;
    const skin = variant ? ["#b98563", "#654638", "#8e6047"] : ["#d5a57b", "#745442", "#9d7054"];
    const shirt = variant ? [set.colors[1], mixColor(set.colors[1], -0.48), mixColor(set.colors[1], -0.26)] : ["#e7d8bd", "#766e61", "#a49a87"];
    box(x + 0.12, y + 0.1, 0.06, 0.3, 0.3, 0.55 + bob, ["#252b30", "#111619", "#1a2024"]);
    box(x + 0.56, y + 0.1, 0.06, 0.3, 0.3, 0.55 - bob, ["#252b30", "#111619", "#1a2024"]);
    box(x, y, 0.55, 1.0, 0.72, 0.75, shirt);
    box(x + 0.23, y + 0.14, 1.3 + bob, 0.55, 0.5, 0.5, skin);
    box(x + 0.17, y + 0.08, 1.75 + bob, 0.67, 0.62, 0.14, [
      variant ? "#25333a" : "#c56a37",
      variant ? "#111a1e" : "#66351f",
      variant ? "#19252a" : "#914925",
    ]);
    const armWave = Math.sin(elapsed * 0.008 + variant) * 0.08;
    box(x - 0.18, y + 0.12 + armWave, 0.73, 0.22, 0.22, 0.65, shirt);
    box(x + 0.96, y + 0.18 - armWave, 0.73, 0.22, 0.22, 0.65, shirt);
  }
  ...
  function drawScene() {
    ...
    add(2.5, () => drawWorker(-3.1, 3.2, set, 0));
    if (state.upgrades.crew > 0) add(4.5, () => drawWorker(1.55, 2.7, set, 1));
