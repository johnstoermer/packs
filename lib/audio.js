  ...
  function sound(name, amount = 0) {
    ...
    switch (name) {
      ...
      case "deal":
        ...
        break;
      case "signal": {
        const order = Math.max(0, Math.min(7, amount));
        const root = [330, 392, 466, 554, 659, 784, 932, 1_108][order];
        tone(root, 0.12 + order * 0.008, {
          type: order > 3 ? "sine" : "triangle",
          to: root * 1.035,
          volume: 0.018 + order * 0.0025,
          wet: 0.12 + order * 0.02,
      ...
      case "reveal": {
        const order = Math.max(0, Math.min(7, amount));
        const root = [196, 246.94, 293.66, 369.99, 440, 523.25, 622.25, 739.99][order];
        noise(0.065, {
          frequency: 2_200 + order * 320,
          volume: 0.06 + order * 0.007,
          smooth: 0.2,
          q: 1.1,
          pan: -0.15,
        });
        tone(88 + order * 9, 0.15, {
          type: "triangle",
          to: 54,
          volume: 0.07 + order * 0.007,
          cutoff: 1_100,
        });
        chord([root, root * 1.25, root * 1.5], 0.2 + order * 0.045, {
          delay: 0.018,
          strum: 0.018,
          type: order >= 3 ? "sine" : "triangle",
          volume: 0.075 + order * 0.006,
          wet: 0.1 + order * 0.035,
          cutoff: 6_500,
        });
        if (order >= 2) {
          tone(root * 2, 0.4 + order * 0.04, {
            delay: 0.06,
            volume: 0.025 + order * 0.003,
            wet: 0.36,
            pan: 0.38,
          });
        }
      ...
      case "legendary":
        impact({ root: 49, power: 0.82 });
        chord([196, 246.94, 293.66, 392], 0.92, {
          delay: 0.04,
          strum: 0.065,
          type: "sine",
          volume: 0.115,
