...
export const SIGNATURES = {
  ...
  verdant: { sig: "fusion", mod: "fusionRule", v: 1, note: "As soon as two same-rarity cards are revealed, Fusion replaces the earlier one with a card of the next rarity. The new card reveals immediately." },
...
const S = {
  corner: [
    ...
    { on: "coinsEarned", every: 100, do: "salvage", chance: 1.5 },
...
export function describeCard(cardId) {
  ...
  if (def.on) {
    ...
    if (def.every) {
      ...
      const clause = def.chance ? `${def.chance}% chance to ${act}` : act;
      return `For every ${noun}, ${clause}.`;
    }
    if (def.coins) {
...
export const CARD_KEYWORD_GLOSSARY = {
  ...
  Scrap: {
    tone: "scrap",
    reminder: "A proposed persistent resource made by Salvaging duplicates.",
    aliases: ["Scrap"],
  },
...
function discoveredCardCount(state) {
  return Object.entries(state.collection || {}).reduce((count, [cardId, copies]) => (
    count + ((copies || 0) > 0 && getCard(cardId) ? 1 : 0)
  ), 0);
}
...
export const CASE_MILESTONES = [
  { slot: 1, label: "Open the shop", met: () => true },
  {
    slot: 2,
    label: "Find 12 cards",
    met: (state) => discoveredCardCount(state) >= 12,
  },
  { slot: 3, label: "Open 75 packs", met: (state) => (state.packsOpened || 0) >= 75 },
  {
    slot: 4,
    label: "Find 36 cards",
    met: (state) => discoveredCardCount(state) >= 36,
  },
  {
    slot: 5,
  ...
  },
  {
    slot: 6,
    label: "Find 72 cards",
    met: (state) => discoveredCardCount(state) >= 72,
  },
];
...
export function getCaseSlots(state) {
  ...
  const highestUnlockedSlot = milestones.reduce(
    (highest, milestone) => (milestone.met ? Math.max(highest, milestone.slot) : highest),
    1,
  );
  return {
    slots: highestUnlockedSlot,
    milestones,
  };
}
