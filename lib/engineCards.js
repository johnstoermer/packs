...
export const SIGNATURES = {
  corner: { sig: "commonEcho", mod: "echoCommonBoost", v: 50, note: "Common reveals have +50% chance to Echo — their reveal effects happen again. Chance past 100% causes additional Echoes." },
  circuit: { sig: "mark", mod: "markEveryPack", v: 1, note: "Every pack you open contains one Marked card, visible before you reveal." },
  frontier: { sig: "salvage", on: "dupSold", salvageChance: 4, note: "Every duplicate you sell has a 4% chance to Salvage — bursting open a free Mystery Pack." },
  abyss: { sig: "mimic", mod: "mimicPack", v: 1, note: "When a pack is opened, one unrevealed card becomes a copy of another unrevealed card." },
  crown: { sig: "rareEcho", mod: "echoRareBoost", v: 50, note: "Rare-or-better reveals have +50% chance to Echo — their reveal effects happen again. Chance past 100% causes additional Echoes." },
  verdant: { sig: "fusion", mod: "fusionRule", v: 1, note: "After a pack is fully revealed, pairs of same-rarity cards Fuse into one card of the next rarity — which reveals again." },
  polar: { sig: "transmute", mod: "transmuteChance", v: 50, note: "Reveals have +50% chance to Transmute an unrevealed card toward the revealed rarity. Chance past 100% affects additional cards." },
  ember: { sig: "fracture", mod: "fractureChance", v: 35, note: "Packs have +35% chance to Fracture, spilling a second pack into the same reveal." },
  cloud: { sig: "catalyst", mod: "catalystChance", v: 75, note: "When an unrevealed card gains a Mark, a copy, or a Transmute: +75% chance it spreads to another unrevealed card. Chance past 100% can spread twice." },
  glass: { sig: "blueprint", mod: "blueprintRule", v: 1, note: "Acts as an exact copy of the card in display slot 1." },
  harbor: { sig: "relay", mod: "relayRule", v: 1, note: "Whenever a displayed card triggers, the displayed card immediately to its right also triggers, if able." },
  orchard: { sig: "autopilot", mod: "autopilotRule", v: 1, note: "Whenever you would Discover, an option is chosen automatically — and automatic choices are enhanced." },
};
...
export const CARD_DEFS = Object.fromEntries(SETS.flatMap((set) => set.cards.map((card) => {
  const [legacySetId, legacyIndex] = splitLegacyId(card.legacy || card.id);
  if (legacyIndex === 11) {
...
const MOD_TEXT = {
  ...
  fractureChance: (v) => `Packs have a +${v}% chance to Fracture, spilling a second pack into the same reveal.`,
  ...
  markExtraChance: (v) => `Packs have a +${v}% chance to contain a Marked card.`,
  markBiasHigh: () => "Marks prefer the highest-rarity card in the pack.",
  markSpread: (v) => `Revealed Marks have a +${v}% chance to spread to an unrevealed card.`,
  markTruth: (v) => `Marked cards' signals are ${v}% less likely to lie.`,
  foilChance: (v) => `Cards have a +${v}% chance to be foil.`,
  ...
  transmuteChance: (v) => `Reveals have a +${v}% chance to Transmute an unrevealed card toward the revealed rarity.`,
  transmuteUp: (v) => `Transmutes climb ${v} additional rarity step${v > 1 ? "s" : ""} upward.`,
  transmuteBiasMarked: () => "Transmutes prefer Marked cards.",
  echoCommonBoost: (v) => `Common reveals have a +${v}% chance to Echo.`,
  echoRareBoost: (v) => `Rare-or-better reveals have a +${v}% chance to Echo.`,
  echoAllBoost: (v) => `All Echo chances are ${v}% higher.`,
  echoChain: (v) => `Echoes have a ${v}% chance to Echo again.`,
  catalystChance: (v) => `Marks, copies, and Transmutes on unrevealed cards have a +${v}% chance to spread to another card.`,
...
export const CASE_MILESTONES = [
  ...
  {
    slot: 2,
    label: "Find 24 cards in one set",
    met: (state) => SETS.some((set) => set.cards.filter((card) => (state.collection?.[card.id] || 0) > 0).length >= 24),
  },
  ...
  },
  {
    slot: 6,
    label: "Finish 3 sets",
    met: (state) => SETS.filter((set) => set.cards.every((card) => (state.collection?.[card.id] || 0) > 0)).length >= 3,
  },
];
