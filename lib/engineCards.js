...
//   salvageChance, do: salvage | discover | boon | packs | saleBoost |
...
//   discoverPersist, discoverConsolation, saleBoost, packDiscount,
...
const S = {
  ...
  orchard: [
    ...
    { mod: "mysteryMarked", v: 1 },
    { on: "fusedReveal", do: "discover", chance: 50 },
    { mod: "discoverConsolation", v: 1 },
  ],
  hollow: [
...
const MOD_TEXT = {
  ...
  fractureMarked: () => "Cards spilled by a Fracture arrive Marked.",
  ...
  fusionFoil: () => "Cards born from Fusion are always foil.",
  ...
  markTruth: () => "Marked cards never bluff their rarity signal.",
  ...
  mimicTwice: () => "Mimic copies a second card.",
  mimicPerfect: () => "Copies are perfect: they keep the original's foil and grade.",
  ...
  mysteryFloor: (v) => `Mystery Pack cards are always ${v >= 2 ? "Rare" : "Uncommon"} or better.`,
  mysteryNewGuarantee: () => "Mystery Packs always include a card you don't own, while any remain.",
  mysteryMarked: () => "Cards from Mystery Packs arrive Marked.",
  ...
  discoverConsolation: () => "Skipped Discovers still grant one random pick.",
  saleBoost: (v) => `Duplicates sell for ${v}% more.`,
  packDiscount: (v) => `Loose packs cost ${v}% less.`,
  thresholdDiscount: (v) => `Cards that trigger every so-many coins or packs trigger ${v}% sooner.`,
  gradeBias: () => "Cards you pull grade one step higher.",
  noFalseSignals: () => "Rarity signals never lie.",
...
export function describeCard(cardId) {
  ...
  if (def.on) {
    ...
    if (def.do === "saleBoost") parts.push(`your next duplicate sale is worth ${def.v}% more`);
    if (def.do === "fuseLift") {
...
export function getEngine(state) {
  ...
  const engine = {
    ...
    discoverOptions: 0,
    discoverPersist: 0,
    discoverConsolation: false,
    saleBoost: 0,
    packDiscount: 0,
    thresholdDiscount: 0,
    gradeBias: 0,
    noFalseSignals: false,
    relayDepth: 1,
    packEncore: null,
  ...
  for (const item of defs) {
    ...
    if (def.mod) {
      switch (def.mod) {
        ...
        case "fractureMarked": engine.fractureMarked = true; break;
        ...
        case "fusionFoil": engine.fusionFoil = true; break;
        ...
        case "markTruth": engine.markTruth = true; break;
        case "mimicBiasHigh": engine.mimicBiasHigh += def.v; break;
        case "mimicBiasMarked": engine.mimicBiasMarked += def.v; break;
        case "mimicTwice": engine.mimicTwice = true; break;
        case "mimicPerfect": engine.mimicPerfect = true; break;
        ...
        case "mysteryMarked": engine.mysteryMarked = true; break;
        ...
        case "discoverConsolation": engine.discoverConsolation = true; break;
        case "saleBoost": engine.saleBoost += def.v; break;
        case "packDiscount": engine.packDiscount += def.v; break;
        case "thresholdDiscount": engine.thresholdDiscount += def.v; break;
        case "gradeBias": engine.gradeBias += def.v; break;
        case "noFalseSignals": engine.noFalseSignals = true; break;
