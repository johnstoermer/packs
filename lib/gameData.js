...
// distribution; the final line swaps its Singularity slot for the Nameless
...
// 48 slots: 10 common, 8 uncommon, 6 rare, 4 epic, 3 legendary, 3 mythic,
// 2 exalted, 2 ascendant, 2 celestial, then one of each deep tier.
const SET_RARITY_LADDER = [
  ...Array(10).fill("common"),
  ...Array(8).fill("uncommon"),
  ...Array(6).fill("rare"),
  ...Array(4).fill("epic"),
  ...Array(3).fill("legendary"),
  ...Array(3).fill("mythic"),
  ...Array(2).fill("exalted"),
  ...Array(2).fill("ascendant"),
  ...Array(2).fill("celestial"),
  "divine", "astral", "eternal", "primordial", "transcendent", "empyrean", "absolute", "singularity",
];

// chaseSlots: legacy set id -> 1-indexed ladder slot for its chase card.
// Signatures land at the top Legendary (31), top Mythic (34), and top
// Celestial (40); capstones take the Absolute (47) and apex (48) slots.
const PRINT_LINES = [
  {
    id: "marquee",
...
// One mega-set, balanced for a 98-card collection while preserving the old
// rarity hierarchy. Cards are ranked by their authored rarity, then spread
// across this distribution so mechanically important cards stay chase cards.
...
for (const rarity of Object.values(CORE_PINNED_RARITIES)) {
  ...
  if (remainingRarityCounts[rarity] < 0) throw new Error(`Too many pinned ${rarity} core cards`);
}
