...
// the real save or cloud sync.
export function createAdminState(now = Date.now()) {
  const state = createInitialState(now);
  for (const set of SETS) state.sealed[set.id].loose = 50;
  const collection = Object.fromEntries(ALL_CARDS.map((card) => [card.id, 1]));
  const bestRarities = Object.fromEntries(ALL_CARDS.map((card) => [card.id, card.rarity]));
  return advanceBeat({
    ...state,
    adminMode: true,
    coins: 1e15,
    ...
    beat: 5,
    packsOpened: 200,
    cardsPulled: ALL_CARDS.length,
    lifetimeStats: { ...state.lifetimeStats, cards: ALL_CARDS.length },
    collection,
    bestRarities,
...
function catalystSpreads(ctx) {
  if (!ctx.engine.kings.has("catalyst")) return 0;
...
function mysteryExtras(ctx) {
  const marked = ctx.engine.mysteryMarked > 0
    && ctx.engine.kings.has("mark")
    && ctx.rng() < ctx.engine.mysteryMarked / 100;
  return marked ? { fromMystery: true, marked: true } : { fromMystery: true };
}
...
function assignMark(ctx, cards, biasHigh) {
  ...
  ctx.events.push({ t: "mark", index: target.index, cardId: ctx.engine.kingSlots.mark || null });
  return target.index;
}
...
export function openPack(state, options = {}) {
  ...
  for (let index = 0; index < Math.min(24, packSize); index += 1) {
    cards.push(polish(rollPackCard(ctx.working, set, rng, context === "sealed" ? null : ctx.engine, usedIds, manual)));
  }
  ...
  if (ctx.engine.kings.has("fracture")) {
    ...
    while (extra < ctx.engine.fractureDepth && rng() < ctx.engine.fractureChance / 100) {
      ...
      for (let index = 0; index < Math.min(24, packSize); index += 1) {
        const spill = polish(rollPackCard(ctx.working, spillSet, rng, ctx.engine, moreUsed, manual));
        if (ctx.engine.fractureMarked > 0 && ctx.engine.kings.has("mark") && rng() < ctx.engine.fractureMarked / 100) {
          spill.marked = true;
...
export function revealPackCard(state, cards, index, options = {}) {
  ...
  const fireMatched = () => {
    for (const record of matched) {
      ...
      if (
        def.spreadMark && revealed.marked && ctx.engine.kings.has("mark")
  ...
  if (
    revealed.marked && engine.kings.has("mark") && engine.markSpread > 0
  ...
  // Echo chance is a real dial: the King grants a base chance, boosts add
  // percentage points, and anything past 100% becomes guaranteed additional
  ...
  if (engine.kings.has("commonEcho") && revealed.rarity === "common") {
    const effective = Math.max(0, (engine.echoCommonChance + echoBonus) * echoAll);
    ...
    const effective = Math.max(0, (engine.echoRareChance + echoBonus) * echoAll);
    echoes = Math.floor(effective / 100) + (rng() * 100 < effective % 100 ? 1 : 0);
