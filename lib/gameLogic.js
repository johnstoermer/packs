...
export const DRY_STREAK_PITY = 18;
...
export function createInitialState(now = Date.now()) {
  ...
  return {
    ...
    settings: {
      ...
      quickOpen: false,
      // Auto-process duplicates: 0 = off, otherwise the pile sells itself
      // once it is worth this many loose packs (threshold, never a timer).
      autoSell: 0,
    },
    createdAt: now,
    lastSavedAt: now,
...
export function getPassiveIncomeRate(state) {
  ...
  return Math.max(BASE_PASSIVE_RATE, newest / 60) * getPrestigeMultiplier(state);
}
...
export function openPack(state, options = {}) {
  ...
  const guaranteeUncommon = (start, fromSet) => {
    ...
    if (previous.marked) cards[at].marked = true;
  };
  ...
  if (context !== "sealed") {
    ...
    if (!hasNew && streak + 1 >= DRY_STREAK_PITY) {
      const missing = set.cards
        .filter((card) => !(ctx.working.collection?.[card.id] > 0))
      ...
      if (missing) {
        ...
        if (previous?.marked) cards[at >= 0 ? at : 0].marked = true;
        hasNew = true;
      }
    }
