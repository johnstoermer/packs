...
function sanitizeBestRarities(raw, collection) {
  ...
  for (const [id, count] of Object.entries(collection)) {
    ...
    const saved = raw?.[id];
    result[id] = saved && RARITIES[saved] ? saved : card.rarity;
  }
  return result;
}

function estimateLegacyDuplicateBank(collection, bestRarities) {
  return Object.entries(collection).reduce((total, [id, count]) => {
    const extras = Math.max(0, count - 1);
    if (!extras) return total;
    const card = getCard(id);
    const rarityId = bestRarities[id] || card?.rarity || "common";
    return total + extras * RARITIES[rarityId].sellValue;
  }, 0);
}
...
export function hydrateState(raw, now = Date.now()) {
  ...
  const state = {
    ...
    duplicateBank: raw.duplicateBank === undefined
      ? estimateLegacyDuplicateBank(collection, bestRarities)
...
export function getPassiveIncomeRate() {
  return BASE_PASSIVE_RATE;
}

export function getCardSaleValue(state, cardId, rarityId = null) {
  const card = getCard(cardId);
  if (!card) return 0;
  const effectiveRarity = rarityId && RARITIES[rarityId]
    ? rarityId
    : (state.bestRarities?.[cardId] || card.rarity);
  return RARITIES[effectiveRarity].sellValue;
}
...
function getRawDuplicateValue(state) {
  ...
  return estimateLegacyDuplicateBank(state.collection || {}, state.bestRarities || {});
}

export function getDuplicateSaleValue(state) {
  const dealerMultiplier = 1 + Math.max(0, finite(state.upgrades?.shelf)) * 0.2;
  return getRawDuplicateValue(state) * dealerMultiplier;
}
...
export function getPackPrice(state, productId = "loose", setId = state.activeSet) {
  ...
  return Math.round(product.costFactor * setPriceScale * (1 - supplierDiscount) * 100) / 100;
}
...
function getUnlockRequirementStatus(state, requirement) {
  if (requirement.type === "card") {
    const card = getCard(requirement.cardId);
    const current = Math.max(0, Math.floor(finite(state.collection?.[requirement.cardId])));
    const target = Math.max(1, Math.floor(finite(requirement.count, 1)));
    return {
      met: current >= target,
      current,
      target,
      label: target === 1 ? `Own ${card?.name || "the required card"}` : `Own ${target} copies of ${card?.name || "the required card"}`,
    };
  }
  if (requirement.type === "packs") {
    const target = Math.max(1, Math.floor(finite(requirement.count, 1)));
    const current = Math.min(target, Math.max(0, Math.floor(finite(state.packsOpened))));
    return { met: current >= target, current, target, label: `Open ${target} packs` };
  }
  if (requirement.type === "completeSet") {
    const set = getSet(requirement.setId);
    const current = set.cards.filter((card) => finite(state.collection?.[card.id]) > 0).length;
    const target = set.cards.length;
    return { met: current >= target, current, target, label: `Finish ${set.name}` };
  }
  if (requirement.type === "completeSets") {
    const target = Math.max(1, Math.floor(finite(requirement.count, 1)));
    const current = Math.min(target, getCompletedSetIds(state).length);
    return { met: current >= target, current, target, label: `Finish ${target} sets` };
  }
  if (requirement.type === "rarityCards") {
    const rarity = RARITIES[requirement.rarity] || RARITIES.mythic;
    const target = Math.max(1, Math.floor(finite(requirement.count, 1)));
    const current = Math.min(target, Object.entries(state.bestRarities || {}).filter(([id, rarityId]) => (
      finite(state.collection?.[id]) > 0
      && RARITIES[rarityId]
      && RARITIES[rarityId].order >= rarity.order
    )).length);
    return { met: current >= target, current, target, label: `Own ${target} ${rarity.label}+ cards` };
  }
...
export function getSetUnlockStatus(state, setId) {
  ...
  const requirements = (set.unlockRequirements || []).map((requirement) => (
    getUnlockRequirementStatus(state, requirement)
  ));
  const qualified = requirements.every((requirement) => requirement.met);
  return {
    unlocked: set.id === "corner" || qualified || state.unlockedSets?.includes(set.id),
    qualified,
    requirements,
  };
}
...
export function advanceBeat(state) {
  const beat = getCurrentBeat(state);
  const unlockedSets = new Set(["corner", ...(state.unlockedSets || [])]);
  for (const set of SETS) {
    if (getSetUnlockStatus({ ...state, unlockedSets: [...unlockedSets] }, set.id).qualified) {
...
export function getUpgradeCost(state, upgradeId) {
  ...
  return Math.round(upgrade.baseCost * upgrade.growth ** rank * 100) / 100;
}
...
function chooseRarity(state, rng) {
  ...
  for (const id of available) {
    ...
    weight *= 1 + Math.max(0, finite(state.upgrades?.lamp)) * 0.05;
...
function chooseCard(set, rarity, rng, usedIds, tagBias) {
  ...
  if (!pool.length) {
    ...
    const highestBaseOrder = Math.max(...set.cards.map((card) => RARITIES[card.rarity].order));
    if (targetOrder > highestBaseOrder) {
      pool = [...set.cards];
    } else {
...
function rollPulls(state, options) {
  ...
  for (let index = 0; index < PACK_SIZE; index += 1) {
    const rarity = chooseRarity(state, rng);
...
function applyPulls(state, pulls, context) {
  ...
  for (const pull of pulls) {
    ...
    const priorRarity = bestRarities[id] || pull.card.rarity;
    const rarityImproved = RARITIES[pull.rarity].order > RARITIES[priorRarity].order;
    ...
    if (rule) {
      ...
      if (priorCount > 0) {
        const saleRarity = rarityImproved ? priorRarity : pull.rarity;
        const saleValue = RARITIES[saleRarity].sellValue;
        duplicateBank += saleValue;
        duplicateValueDelta += saleValue;
        duplicatesAdded += 1;
      }
      collection[id] = priorCount + 1;
      if (rarityImproved || !bestRarities[id]) bestRarities[id] = rarityImproved ? pull.rarity : priorRarity;
      pull.rarityAfter = bestRarities[id];
...
export function applyOfflineProgress(state, now = Date.now()) {
  ...
  const income = getPassiveIncomeRate(state) * elapsedSeconds;
  const next = { ...addPassiveIncome(state, income), lastSavedAt: now };
  return {
    state: next,
    report: income > 0
