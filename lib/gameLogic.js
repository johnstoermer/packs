...
function migrateLegacySave(raw) {
  ...
  delete next.misprints;
  delete next.dormantMisprints;
...
export function hydrateState(raw, now = Date.now()) {
  ...
  delete state.misprints;
  delete state.dormantMisprints;
...
export function sellDuplicatesDetailed(state, options = {}) {
  ...
  if (!duplicateCount) {
    return {
      state,
      cards: Array.isArray(options.injectCards) ? options.injectCards : null,
      events: [],
      mysteryCards: [],
      saleValue: 0,
      saleItems: [],
  ...
  const injectedStart = Array.isArray(options.injectCards) ? options.injectCards.length : 0;
  ctx.injectCards = Array.isArray(options.injectCards)
    ? options.injectCards.slice()
    : null;
  ...
  if (chance > 0) {
    ...
    if (salvages > 0) performSalvage(ctx, salvages, ctx.injectCards, 0);
  }
  for (const record of engine.onSaleMade) {
    tallyTrigger(ctx, record.id);
    fireSupportPayoff(ctx, record, null, ctx.injectCards, 0);
    relayCascade(ctx, record.slot, null, ctx.injectCards, 0);
  }
  ctx.events.push({ t: "sold", count: duplicateCount, value: saleValue, items: saleItems });
  return {
    state: finishEngineContext(ctx),
    cards: ctx.injectCards,
    events: ctx.events,
    mysteryCards: ctx.injectCards
      ? ctx.injectCards.slice(injectedStart).filter((pull) => pull.fromMystery || pull.fromFracture)
      : ctx.mysteryDirect,
    saleValue,
    saleItems,
    salvages,
...
// Slot order is load-bearing — Blueprint copies slot 1, Relay fires rightward,
// triggerLeft fires leftward — so the case lets you rearrange it. Watermark
// credit is keyed by card id rather than slot, so moving a card carries its
// progress with it and reordering can neither bank nor burn a threshold.
// Pending duplicates settle first, at the arrangement that earned them.
export function reorderDisplayed(state, fromIndex, toIndex, now = Date.now()) {
  const from = Math.floor(finite(fromIndex, -1));
  const to = Math.floor(finite(toIndex, -1));
  const entries = getDisplayedEntries(state);
  if (from === to || from < 0 || to < 0 || from >= entries.length || to >= entries.length) return state;
  const working = getDuplicateCount(state) > 0 ? sellDuplicates(state, now) : state;
  const current = getDisplayedEntries(working);
  if (from >= current.length || to >= current.length) return working;
  const next = [...current];
  next.splice(to, 0, ...next.splice(from, 1));
  return { ...working, displayed: next };
}
...
function addCardsToOpenedPack(ctx, focusPull, injectCards, count, rarity = null) {
  if (!Array.isArray(injectCards) || !focusPull?.card) return 0;
...
function revealRandomDuplicate(ctx, sourceId, injectCards, depth) {
  ...
  applyRevealToCollection(ctx, pull);
  if (Array.isArray(injectCards)) injectCards.push(pull);
...
function assignMark(ctx, cards, biasHigh) {
  ...
  // Card buffers can contain thousands of queued pulls. They are shallow
  // copy-on-write arrays, so clone only the selected pull before changing it;
  // cloning every off-screen object made one visible flip allocate the entire
  // backlog several times.
  const markedPull = { ...target.pull };
  cards[target.index] = markedPull;
  markedPull.marked = true;
  markedPull.markStacks = Math.max(0, Math.floor(finite(markedPull.markStacks))) + 1;
  if (ctx.engine.markTruth > 0 && markedPull.falseSignal && ctx.rng() < ctx.engine.markTruth / 100) {
    markedPull.falseSignal = false;
    markedPull.signalRarity = markedPull.rarity;
  }
  ctx.events.push({
    t: "mark",
    index: target.index,
    stacks: markedPull.markStacks,
...
// Picks the cards that belong on one reveal screen. A screen holds `size`
// cards that were still face-down when it opened, so a big pack is dealt out
// a screenful at a time.
//
// `filed` carries the indices that were already face-up when this screen
// opened. They are skipped rather than seated: effects like revealRest, and
// generated duplicates that arrive face-up, can turn cards that live past the
// screen cap, and seating those on a later screen would open it with one card
// left to turn and the rest already revealed. They are collected at the screen
// boundary instead. Cards turned *during* the current screen are absent from
// `filed`, so they stay put until that screen is done.
export function planRevealBatch(cards = [], start = 0, size = 1, filed = null) {
  const limit = Math.max(1, Math.floor(finite(size, 1)));
  const from = Math.max(0, Math.floor(finite(start)));
  const indices = [];
  for (let index = from; index < cards.length && indices.length < limit; index += 1) {
    const pull = cards[index];
    if (!pull || pull.fusedAway || filed?.has(index)) continue;
    indices.push(index);
  }
  return indices;
}

// The indices to hand the next screen as `filed`: everything already resolved
// at the moment one screen hands off to the next.
export function getFiledIndices(cards = []) {
  const filed = new Set();
  for (let index = 0; index < cards.length; index += 1) {
    const pull = cards[index];
    if (pull && (pull.revealed || pull.fusedAway)) filed.add(index);
  }
  return filed;
}
...
export function getPendingCardCount(cards = []) {
  let count = 0;
  for (const pull of cards) {
    if (pull && !pull.revealed && !pull.fusedAway) count += 1;
  }
  return count;
}
...
export function revealPackCard(state, cards, index, options = {}) {
  ...
  const carriedEchoBoost = Math.max(0, finite(revealed.echoBoost));
  let echoes = 0;
  let echoKingId = null;
  if (revealed.rarity === "common") {
    ...
    const effective = Math.max(0, engine.echoMarkedChance + engine.echoAllBoost + carriedEchoBoost);
    echoes = Math.floor(effective / 100) + (rng() * 100 < effective % 100 ? 1 : 0);
  } else if (engine.echoAllBoost + carriedEchoBoost > 0) {
    const effective = Math.max(0, engine.echoAllBoost + carriedEchoBoost);
    echoes = Math.floor(effective / 100) + (rng() * 100 < effective % 100 ? 1 : 0);
  }
  if (echoes > 0) {
...
// Same-rarity pairs can resolve either at the legacy end-of-pack boundary or
// immediately after a reveal. Immediate resolution replaces the earlier slot
// and consumes the later slot in place. Stable indices are load-bearing for a
// staged opening: splicing a large buffer shifted every later pull and made
// React reconcile most of the visible screen after every Fusion.
export function resolveFusions(state, cards, options = {}) {
  ...
  if (!immediate && cards.some((pull) => !pull.revealed)) {
    return { state, cards, events: [], fused: false };
  }
  ...
  const fuseInto = (rarity, left, right) => {
    const destination = right === null ? left : Math.min(left, right);
    const consumed = right === null ? null : Math.max(left, right);
    const parentSet = getSet(nextCards[destination].card.setId);
    ...
    if (catalystStack === null) catalystStack = consumeDiscoverStack(ctx, "catalyst");
    ...
    let resultIndex;
    if (immediate) {
      nextCards[destination] = result;
      if (consumed !== null) {
        nextCards[consumed] = { ...nextCards[consumed], fusedAway: true };
      }
      resultIndex = destination;
    } else {
      ...
      resultIndex = nextCards.length - 1;
    }
    ctx.events.push({
      t: "fusion",
      from: rarity,
      to: fusedCard.rarity,
      left: destination,
      right: consumed ?? destination,
      index: resultIndex,
      cardId: engine.kingSlots.fusion || null,
      resultCardId: fusedCard.id,
    });
    fused = true;
    ctx.working.lifetimeStats.fusions += 1;
    let carriedEchoBoost = 0;
    for (const record of engine.onFusion) {
      tallyTrigger(ctx, record.id);
      const def = fireSupportPayoff(ctx, record, result, nextCards, 0);
      carriedEchoBoost += Math.max(0, finite(def.echoBoost));
      relayCascade(ctx, record.slot, result, nextCards, 0);
    }
    if (carriedEchoBoost > 0) result.echoBoost = carriedEchoBoost;
    return true;
  };

  if (immediate) {
    const active = nextCards
      .map((pull, index) => ({ pull, index }))
      .filter(({ pull }) => pull.revealed && !pull.fusedAway);
    const byRarity = new Map();
    for (const entry of active) {
      const list = byRarity.get(entry.pull.rarity) || [];
      list.push(entry.index);
      byRarity.set(entry.pull.rarity, list);
    }

    if (engine.fusionRule) {
      for (const [rarity, indices] of byRarity) {
        if (indices.length >= 2 && fuseInto(rarity, indices[0], indices[1])) break;
      }
    }

    if (!fused && engine.fusionCross) {
      const singles = [...byRarity]
        .filter(([, indices]) => indices.length === 1)
        .map(([rarity, indices]) => ({ rarity, index: indices[0] }))
        .sort((left, right) => (
          RARITIES[left.rarity].order - RARITIES[right.rarity].order
          || left.index - right.index
        ));
      for (let index = 0; index < singles.length - 1; index += 1) {
        const low = singles[index];
        const high = singles[index + 1];
        if (RARITIES[high.rarity].order - RARITIES[low.rarity].order !== 1) continue;
        if (fuseInto(high.rarity, low.index, high.index)) {
    ...
    if (!fused && engine.fusionSolo > 0 && active.length && rng() < engine.fusionSolo / 100) {
      const pick = active[Math.floor(rng() * active.length) % active.length];
      if (fuseInto(pick.pull.rarity, pick.index, null)) {
        ctx.events.push({ t: "fusionSolo", rarity: pick.pull.rarity });
      }
    }
    ...
    evaluateStatThresholds(ctx, 0);
    return {
      state: advanceBeat(finishEngineContext(ctx)),
      cards: nextCards,
      events: ctx.events,
      fused,
    };
...
// FINISH path: reveal every remaining card in one pass with display effects
// suppressed. Clones the table exactly once, so arbitrarily large flooded
// reveals collect in linear time instead of freezing the game.
export function revealAllPackCards(state, cards) {
  const ctx = makeEngineContext(state, getEngine({ ...state, displayed: [] }), Math.random);
  const nextCards = cards.map((entry) => ({ ...entry }));
  ctx.injectCards = nextCards;
  const setsBefore = new Map();
  const setComplete = (setId) => getSet(setId).cards.every(
    (candidate) => finite(ctx.working.collection[candidate.id]) > 0,
  );
  let dupStreak = Math.max(0, Math.floor(finite(ctx.working.counters.dupStreak)));
  for (let index = 0; index < nextCards.length; index += 1) {
    const pull = nextCards[index];
    if (!pull || pull.revealed || pull.fusedAway) continue;
    const setId = pull.card.setId;
    if (!setsBefore.has(setId)) setsBefore.set(setId, setComplete(setId));
    applyRevealToCollection(ctx, pull);
    ctx.events.push({ t: "reveal", index, cardId: pull.card.id, rarity: pull.rarity, isNew: pull.isNew });
    dupStreak = pull.isNew ? 0 : dupStreak + 1;
  }
  ctx.working.counters = { ...ctx.working.counters, dupStreak };
  for (const [setId, wasComplete] of setsBefore) {
    if (!wasComplete && setComplete(setId)) ctx.events.push({ t: "setComplete", setId });
  }
  evaluateStatThresholds(ctx, 0);
  return { state: advanceBeat(finishEngineContext(ctx)), cards: nextCards, events: ctx.events };
}

// One immediate Fusion transaction includes the replacement card's normal
// reveal. This guarantees that its printed reveal effects and display-card
// triggers run exactly as if the player had flipped that slot.
export function resolveImmediateFusion(state, cards, options = {}) {
  const fusion = resolveFusions(state, cards, { ...options, immediate: true });
  if (!fusion.fused) return fusion;
  const event = fusion.events.findLast((entry) => entry.t === "fusion");
  if (!event || !fusion.cards[event.index]) return fusion;
  const revealed = revealPackCard(fusion.state, fusion.cards, event.index, options);
  return {
    state: revealed.state,
    cards: revealed.cards,
    events: [...fusion.events, ...revealed.events],
    fused: true,
    index: event.index,
    cardId: event.resultCardId,
  };
}
