...
function enqueue(ctx, action) {
  if (ctx.queue.length >= MAX_QUEUE_LENGTH) return false;
...
function applySalvage(ctx, action) {
  ...
  ctx.events.push({ t: "salvage", index: action.index, cardId: pull.card.id, amount, source: action.source || null });
...
// it, and returns. This is the only place actions resolve; callers pace the
// game by how often they step.
