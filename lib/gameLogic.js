...
export function hydrateState(raw, now = Date.now()) {
  ...
  state.lastDuplicateSaleAt = Math.max(0, finite(raw.lastDuplicateSaleAt));
...
function getBankInterestMultiplier(state, now) {
  const rate = getDisplayModifiers(state, now).bankInterest;
  if (rate <= 0) return 1;
  const since = Math.max(0, finite(state.lastDuplicateSaleAt)) || finite(state.createdAt, now);
  const minutes = Math.max(0, (now - since) / 60_000);
  return Math.min(2, 1 + (rate / 100) * minutes);
}

export function getDuplicateSaleValue(state, now = Date.now()) {
  const dealerMultiplier = 1 + Math.max(0, finite(state.upgrades?.shelf)) * 0.2;
  const displayMultiplier = 1 + getDisplayModifiers(state, now).dupValue / 100;
  const rawValue = getRawDuplicateValue(state)
    * dealerMultiplier
    * displayMultiplier
    * getBankInterestMultiplier(state, now)
...
export function sellDuplicates(state, now = Date.now()) {
  ...
  const saleValue = getDuplicateSaleValue(state, now);
...
export function rewriteState(state, now = Date.now()) {
  ...
  if (mods.keepDisplayed) {
    const entries = getDisplayedEntries(state);
    fresh.displayed = entries.map((entry) => ({ id: entry.id, at: now }));
    for (const entry of entries) {
      fresh.collection[entry.id] = 1;
      fresh.bestRarities[entry.id] = getCard(entry.id).rarity;
    }
  }
  return advanceBeat(fresh);
}

export function getPackPrice(state, productId = "loose", setId = state.activeSet, now = Date.now()) {
...
export function buyProduct(state, productId, setId = state.activeSet, quantity = 1, now = Date.now()) {
  const product = PACK_PRODUCTS.find((candidate) => candidate.id === productId);
  ...
  const unitPrice = getPackPrice(state, productId, setId, now);
  const priorPurchases = Math.max(0, Math.floor(finite(state.packsPurchased)));
  let freeUnits = 0;
  if (productId === "loose") {
    const everyNth = getDisplayModifiers(state, now).buyBulkFree;
    if (everyNth > 0) {
      for (let unit = 1; unit <= count; unit += 1) {
        if ((priorPurchases + unit) % everyNth === 0) freeUnits += 1;
      }
    }
  }
  const cost = unitPrice * (count - freeUnits);
...
export function tickEconomy(state, seconds, now = Date.now()) {
  const elapsed = Math.max(0, Math.min(2, finite(seconds)));
  const interest = getDisplayModifiers(state, now).interest;
  const interestPerSecond = interest > 0
    ? (Math.max(0, finite(state.coins)) * interest) / 100 / 60
    : 0;
  return addPassiveIncome(state, (getPassiveIncomeRate(state, now) + interestPerSecond) * elapsed);
}
...
function gradeCard(rng, manual, gradeFloor = 0) {
  if (!manual) return 4;
  const roll = rng();
  let grade = 5;
  if (roll > 0.985) grade = 10;
  else if (roll > 0.91) grade = 9;
  else if (roll > 0.70) grade = 8;
  else if (roll > 0.38) grade = 7;
  else if (roll > 0.12) grade = 6;
  return Math.max(grade, Math.min(EFFECT_CAPS.gradeFloor, gradeFloor));
}

function signalRarity(rarity, state, rng, trueSignal = false) {
  const available = availableRarities(state.beat);
  const index = available.indexOf(rarity);
  const bluffs = rng() < 0.12;
  if (!trueSignal && bluffs && index >= 0 && index < available.length - 1) {
...
function rollPulls(state, options) {
  ...
  let packSize = PACK_SIZE + (mods.extraCard > 0 && rng() < mods.extraCard / 100 ? 1 : 0);
  if (godPack && mods.godExtraCard) packSize += 1;
  ...
  const pityFloorOrder = Math.min(
    mods.pityPower ? RARITIES.epic.order : RARITIES.rare.order,
    chaseOrder,
  );

  for (let index = 0; index < packSize; index += 1) {
    let pullSet = set;
    if (!godPack && mods.crossSetHunt > 0 && rng() < mods.crossSetHunt / 100) {
      const unfinished = state.unlockedSets.filter((id) => (
        id !== set.id
        && getSet(id).cards.some((candidate) => !(state.collection?.[candidate.id] > 0))
      ));
      if (unfinished.length) {
        pullSet = getSet(unfinished[Math.floor(rng() * unfinished.length) % unfinished.length]);
      }
    }
    if (pullSet === set && !godPack && mods.crossSet > 0 && state.unlockedSets.length > 1 && rng() < mods.crossSet / 100) {
    ...
    if (godPack) {
      ...
      rolledRarity = raiseRarityToOrder(rolledRarity, pityFloorOrder);
    }
    ...
    usedIds.add(card.id);
    const signal = signalRarity(rarity, state, rng, mods.trueSignal);
    const misprint = rng() < 0.0075;
    const foil = rng() < 0.018 + mods.foilChance / 100;
    cards.push({
      card,
      rarity,
      foil,
      grade: gradeCard(rng, manual, mods.gradeFloor),
      misprint,
...
export function openPack(state, options = {}) {
  ...
  const bonusEvents = [];
  if (context !== "sealed") {
    const addBonus = (label, amount) => {
      if (amount <= 0) return;
      bonusCash += amount;
      const existing = bonusEvents.find((event) => event.label === label);
      if (existing) existing.amount += amount;
      else bonusEvents.push({ label, amount });
    };
    for (const pull of rolled.cards) {
      const sellValue = RARITIES[pull.card.rarity].sellValue;
      if (pull.isNew && mods.newCardCash > 0) addBonus("NEW CARD", mods.newCardCash * sellValue);
      if (!pull.isNew && mods.dupCash > 0) addBonus("DUPLICATE CUT", (mods.dupCash / 100) * sellValue);
      for (const tier of mods.rarityCash) {
        if (RARITIES[pull.rarity].order >= tier.minOrder) addBonus("PREMIUM PULL", tier.mult * sellValue);
      }
      if (pull.foil && mods.foilCash > 0) addBonus("FOIL", mods.foilCash * sellValue);
      if (pull.misprintDetected && mods.misprintCash > 0) addBonus("MISPRINT", mods.misprintCash * sellValue);
    }
    const packPrice = getPackPrice(state, "loose", setId);
    const lastManual = Math.max(0, finite(state.lastManualAt));
    if (manual && mods.quickCash > 0 && lastManual > 0 && openedAt - lastManual <= QUICK_OPEN_WINDOW_MS) {
      addBonus("QUICK HANDS", mods.quickCash);
    }
    if (
      manual
      && mods.firstPackCash > 0
      && (lastManual === 0 || openedAt - lastManual >= FIRST_PACK_IDLE_MS)
    ) {
      addBonus("FIRST THROUGH THE FENCE", mods.firstPackCash * packPrice);
    }
    if (mods.allDupRefund > 0 && rolled.cards.every((pull) => !pull.isNew)) {
      addBonus("FARE REFUND", (mods.allDupRefund / 100) * packPrice);
    }
    if (mods.completionCash > 0) {
      const touchedSets = [...new Set(rolled.cards.map((pull) => pull.card.setId))];
      for (const touchedId of touchedSets) {
        const touched = getSet(touchedId);
        const wasComplete = touched.cards.every((candidate) => finite(state.collection?.[candidate.id]) > 0);
        const nowComplete = touched.cards.every((candidate) => finite(working.collection?.[candidate.id]) > 0);
        if (!wasComplete && nowComplete) {
          addBonus("SET FINISHED", mods.completionCash * Math.max(1, finite(touched.baseValue, 1)));
        }
      }
    }
    bonusCash = Math.floor(bonusCash);
    if (bonusCash > 0) {
      working = {
        ...working,
        coins: Math.floor(Math.max(0, finite(working.coins))) + bonusCash,
        lifetimeCoins: Math.floor(Math.max(0, finite(working.lifetimeCoins))) + bonusCash,
      };
    }
  }
...
export function applyOfflineProgress(state, now = Date.now()) {
  ...
  const mods = getDisplayModifiers(state, now);
  const offlineMultiplier = 1 + mods.offline / 100;
  let next = {
  ...
  let offlineOpened = 0;
  let offlineNewCards = 0;
  let packBudget = Math.min(mods.offlinePacks, Math.floor(elapsedSeconds / 60));
  while (packBudget > 0 && getProductCount(next, next.activeSet, "loose") > 0) {
    const rolled = openPack(next, { manual: false, source: "loose", now });
    if (!rolled.result) break;
    next = { ...rolled.state, lastSavedAt: now };
    offlineOpened += 1;
    offlineNewCards += rolled.result.cards.filter((pull) => pull.isNew).length;
    packBudget -= 1;
  }

  return {
    state: next,
    report: income > 0 || offlineOpened > 0
      ? { seconds: elapsedSeconds, coins: income, ordered: 0, packsOpened: offlineOpened, newCards: offlineNewCards }
      : null,
  };
}
