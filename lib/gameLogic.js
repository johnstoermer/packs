...
export function getDuplicateSaleValue(state) {
  ...
  const engineMultiplier = 1 + (
    Math.max(0, engine.saleBoost) + Math.max(0, finite(state.counters?.saleBoost))
  ) / 100;
  const rawValue = getRawDuplicateValue(state) * dealerMultiplier * engineMultiplier * getPrestigeMultiplier(state);
...
export function sellDuplicatesDetailed(state, options = {}) {
  ...
  if (finite(ctx.working.counters.saleBoost) > 0) {
    const counters = { ...ctx.working.counters };
    delete counters.saleBoost;
...
export function getPackPrice(state, productId = "loose", setId = state.activeSet) {
  ...
  const engineDiscount = productId === "loose" ? Math.max(0, getEngine(state).packDiscount) / 100 : 0;
  const discount = Math.min(0.7, supplierDiscount + engineDiscount);
  return Math.max(1, Math.floor(product.costFactor * setPriceScale * (1 - discount)));
}
...
function fireDiscovered(state, engine) {
  let next = state;
  for (const record of engine.onDiscovered) {
    let gain = 0;
    if (record.def.coinsPack) gain += record.def.coinsPack * getPackPrice(next, "loose", next.activeSet);
    if (record.def.coinsFlat) gain += record.def.coinsFlat;
    if (gain > 0) {
      next = {
        ...next,
        coins: Math.floor(Math.max(0, finite(next.coins))) + Math.floor(gain),
        lifetimeCoins: Math.floor(Math.max(0, finite(next.lifetimeCoins))) + Math.floor(gain),
        triggerTallies: { ...next.triggerTallies, [record.id]: (next.triggerTallies?.[record.id] || 0) + 1 },
      };
    }
  }
  return next;
...
function offerDiscover(ctx, sourceId) {
  ...
  if (ctx.engine.kings.has("autopilot")) {
    ...
    for (const record of ctx.engine.onDiscovered) {
      tallyTrigger(ctx, record.id);
      if (record.def.coinsPack) {
        addEngineCoins(ctx, record.def.coinsPack * getPackPrice(ctx.working, "loose", ctx.working.activeSet), record.id, 2);
      }
      if (record.def.coinsFlat) addEngineCoins(ctx, record.def.coinsFlat, record.id, 2);
    }
    return;
  }
...
function mysteryExtras(ctx) {
  return ctx.engine.mysteryMarked && ctx.engine.kings.has("mark")
    ? { fromMystery: true, marked: true }
    : { fromMystery: true };
}
...
function rollMysteryCard(ctx) {
  ...
  let rarity = chooseRarity(state, ctx.rng);
  if (ctx.engine.mysteryFloor > 0 && RARITIES[rarity].order < ctx.engine.mysteryFloor) {
  ...
  return makePull(state, card, ctx.rng, true, mysteryExtras(ctx));
}
...
function performSalvage(ctx, times, injectCards, depth) {
  ...
  while (total > 0) {
    ...
    if (ctx.engine.mysteryNewGuarantee && !cards.some((pull) => !(ctx.working.collection[pull.card.id] > 0))) {
...
function fireSupportPayoff(ctx, record, pull, injectCards, depth) {
  ...
  if (def.do === "saleBoost") {
    ctx.working.counters = {
      ...ctx.working.counters,
      saleBoost: Math.min(500, Math.max(0, finite(ctx.working.counters.saleBoost)) + def.v),
    };
    ctx.events.push({ t: "saleBoost", amount: def.v, source: record.id });
  }
...
export function openPack(state, options = {}) {
  ...
  let packSize = PACK_SIZE + (context === "sealed" ? 0 : ctx.engine.packSize);
  if (context !== "sealed") {
  ...
  const polishPull = (pull) => {
    if (context === "sealed") return pull;
    if (ctx.engine.noFalseSignals) {
      pull.falseSignal = false;
      pull.signalRarity = pull.rarity;
    }
    if (ctx.engine.foilChance > 0 && !pull.foil && rng() < ctx.engine.foilChance / 100) pull.foil = true;
    if (ctx.engine.gradeBias > 0 && manual) pull.grade = Math.min(10, pull.grade + ctx.engine.gradeBias);
    return pull;
  };
  const usedIds = new Set();
  const cards = [];
  for (let index = 0; index < Math.min(24, packSize); index += 1) {
    cards.push(polishPull(rollPackCard(ctx.working, set, rng, context === "sealed" ? null : ctx.engine, usedIds, manual)));
  }
  ...
  if (ctx.engine.kings.has("fracture")) {
    ...
    while (extra < ctx.engine.fractureDepth && rng() < ctx.engine.fractureChance / 100) {
      ...
      for (let index = 0; index < Math.min(24, packSize); index += 1) {
        const spill = polishPull(rollPackCard(ctx.working, spillSet, rng, ctx.engine, moreUsed, manual));
        if (ctx.engine.fractureMarked && ctx.engine.kings.has("mark")) {
          spill.marked = true;
          if (ctx.engine.markTruth) {
  ...
  if (ctx.engine.kings.has("mimic") && cards.length >= 2) {
    ...
    if (targets.length) {
      const applyMimic = (entry) => {
        const perfect = ctx.engine.mimicPerfect
...
export function revealPackCard(state, cards, index, options = {}) {
  ...
  if (
    ...
    for (let count = 0; count < engine.packEncore.n; count += 1) {
      const bonus = rollPackCard(ctx.working, encoreSet, rng, engine, usedIds, manual);
...
export function resolveFusions(state, cards, options = {}) {
  ...
  const fuseInto = (rarity, left, right) => {
    ...
    const extras = { fusedFrom: rarity, ...(engine.fusionFoil ? { foil: true } : {}) };
