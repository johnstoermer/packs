...
export const BINDER_PAYOUT_SCALE = 0.02;
...
export const SET_PACK_THRESHOLDS = {
  corner: 0,
  circuit: 150,
  frontier: 500,
  abyss: 1_500,
...
export function getBinderIncome(state) {
  const base = ALL_CARDS.reduce((total, card) => total + getCardIncome(state, card.id), 0);
  const shelfMultiplier = 1 + Math.max(0, finite(state.upgrades?.shelf)) * 0.2;
  return base * BINDER_PAYOUT_SCALE * shelfMultiplier;
}
...
export function getPackPrice(state, productId = "loose") {
  ...
  return Math.round(product.costFactor * marketPressure * (1 - supplierDiscount) * 100) / 100;
}

function getProductMarketValue(state, productId = "loose") {
  const product = PACK_PRODUCTS.find((candidate) => candidate.id === productId);
  if (!product) return 0;
  const marketPressure = 1.0008 ** Math.max(0, finite(state.packsOpened));
  return product.costFactor * marketPressure;
}
...
export function getSealedAssetValue(state) {
  ...
  for (const set of SETS) {
    for (const product of PACK_PRODUCTS) {
      value += getProductCount(state, set.id, product.id) * getProductMarketValue(state, product.id);
    }
    for (const tag of ARCHETYPES) {
      value += getForgedCount(state, set.id, tag.id) * getProductMarketValue(state, "loose");
    }
  }
  return value;
}
...
export function advanceBeat(state) {
  ...
  const unlockedSets = SETS
    .filter((set) => state.packsOpened >= SET_PACK_THRESHOLDS[set.id])
    .map((set) => set.id);
...
export function tickEconomy(state, seconds) {
  ...
  return addPassiveIncome(state, getBinderIncome(state) * elapsed);
}
...
export function openPack(state, options = {}) {
  ...
  const binderIncomeBefore = getBinderIncome(working);
  ...
  return {
    ...
    result: {
      ...
      source,
      context,
      incomeDelta: Math.max(0, getBinderIncome(working) - binderIncomeBefore),
      fusionEvents,
