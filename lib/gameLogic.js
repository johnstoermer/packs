...
function cloneStock(stock) {
  return Object.fromEntries(
    SETS.map((set) => [
      set.id,
      Object.fromEntries(PACK_PRODUCTS.map((product) => [
        product.id,
...
export function hydrateState(raw, now = Date.now()) {
  ...
  const forged = cloneForgeStock(raw.forged);
  ...
  const state = {
    ...
    coins: Math.max(0, finite(raw.coins)),
    lifetimeCoins: Math.max(0, finite(raw.lifetimeCoins)),
    ...
    standingOrder: { ...initial.standingOrder, ...(raw.standingOrder || {}) },
    filingRules: Array.isArray(raw.filingRules) ? raw.filingRules.slice(0, 6) : [],
    ...
    sealedRun: sanitizeSealedRun(raw.sealedRun),
...
export function getCardIncome(state, cardId) {
  ...
  const count = Math.max(0, Math.floor(finite(state.collection?.[cardId])));
  if (!card || !count) return 0;
  const misprintBoost = state.misprints?.[cardId] ? 1 + Math.min(1, state.misprints[cardId] * 0.2) : 1;
  return RARITIES[card.rarity].income * getFusionMultiplier(count) * misprintBoost;
}

export function getBinderIncome(state) {
  if (state.beat < 2) return 0;
  return ALL_CARDS.reduce((total, card) => total + getCardIncome(state, card.id), 0);
}

export function getPackPrice(state, productId = "loose") {
  const product = PACK_PRODUCTS.find((candidate) => candidate.id === productId);
  if (!product) return Number.POSITIVE_INFINITY;
  const marketPressure = 1.0008 ** Math.max(0, finite(state.packsOpened));
  return Math.round(product.costFactor * marketPressure * 100) / 100;
}
...
export function getSealedAssetValue(state) {
  let value = 0;
  for (const set of SETS) {
    for (const product of PACK_PRODUCTS) {
      value += getProductCount(state, set.id, product.id) * getPackPrice(state, product.id);
    }
    for (const tag of ARCHETYPES) {
      value += getForgedCount(state, set.id, tag.id) * getPackPrice(state, "loose");
    }
  }
  return value;
}
...
export function getCurrentBeat(state) {
  let beat = 1;
  if (state.packsOpened >= 3) beat = 2;
  const unique = Object.keys(state.collection || {}).filter((id) => state.collection[id] > 0).length;
  if (state.packsOpened >= 6 && unique >= 8) beat = 3;
  if (state.duelsWon >= 1) beat = 4;
  if (state.sealedWins >= 1) beat = 5;
  return Math.max(beat, Math.min(5, Math.floor(finite(state.beat, 1))));
}

export function getBeatProgress(state) {
  const beat = getCurrentBeat(state);
  if (beat === 1) return { value: Math.min(state.packsOpened, 3), max: 3, label: "Open 3 packs" };
  if (beat === 2) {
    const unique = Object.keys(state.collection || {}).filter((id) => state.collection[id] > 0).length;
    return {
      value: Math.min(14, Math.min(state.packsOpened, 6) + Math.min(unique, 8)),
      max: 14,
      label: `${Math.min(state.packsOpened, 6)}/6 packs · ${Math.min(unique, 8)}/8 discoveries`,
    };
  }
  if (beat === 3) return { value: Math.min(state.duelsWon, 1), max: 1, label: "Win a constructed duel" };
  if (beat === 4) return { value: Math.min(state.sealedWins, 1), max: 1, label: "Win a sealed match" };
  return { value: 1, max: 1, label: "v1 complete · optimize the binder" };
}

export function advanceBeat(state) {
  const beat = getCurrentBeat(state);
  const unlockedSets = SETS
    .filter((set) => SET_AVAILABILITY[set.id] <= beat)
    .map((set) => set.id);
  if (!unlockedSets.includes("corner")) unlockedSets.unshift("corner");
  const activeSet = unlockedSets.includes(state.activeSet) ? state.activeSet : "corner";
  return {
    ...state,
    beat,
    activeSet,
...
export function getDerived(state) {
  const passiveRate = getBinderIncome(state);
  ...
  return {
    ...
    discoveredCount,
    fusionStars,
    sealedPacks: getSealedPackCount(state),
    sealedAssetValue: getSealedAssetValue(state),
    heatMultiplier: 1.004 ** Math.max(0, state.pityLegendary),
    heatPercent: (1.004 ** Math.max(0, state.pityLegendary) - 1) * 100,
...
export function buyProduct(state, productId, setId = state.activeSet, quantity = 1) {
  ...
  const cost = getPackPrice(state, productId) * count;
...
export function tickEconomy(state, seconds) {
  ...
  let next = addPassiveIncome(state, getBinderIncome(state) * elapsed);
  const order = next.standingOrder;
  if (next.beat >= 2 && order?.enabled) {
    const price = getPackPrice(next, order.product);
    if (next.coins - order.reserve >= price) {
      const bought = buyProduct(next, order.product, order.setId, 1);
      if (bought !== next) {
        next = {
          ...bought,
          standingOrder: {
            ...bought.standingOrder,
            purchased: Math.max(0, Math.floor(finite(bought.standingOrder.purchased))) + 1,
          },
        };
      }
    }
...
function chooseRarity(state, rng, hitSlot, manual) {
  const available = availableRarities(state.beat).sort((a, b) => RARITIES[b].order - RARITIES[a].order);
  const heat = manual ? 1.004 ** Math.max(0, state.pityLegendary) : 1;
  let roll = rng();
  for (const id of available) {
    if (id === "common") continue;
    const rarity = RARITIES[id];
    let weight = rarity.odds;
    if (hitSlot && manual) weight *= rarity.order >= RARITIES.rare.order ? 1.25 : 1.08;
    if (manual && rarity.order >= RARITIES.legendary.order) weight *= heat;
...
function rollPulls(state, options) {
  ...
  for (let index = 0; index < PACK_SIZE; index += 1) {
    const rarity = chooseRarity(state, rng, index === PACK_SIZE - 1, manual);
    const card = chooseCard(set, rarity, rng, usedIds, tagBias);
    usedIds.add(card.id);
...
function applyPulls(state, pulls, context) {
  ...
  for (const pull of pulls) {
    ...
    incomeBefore += getCardIncome({ ...state, collection }, id);
    ...
    incomeAfter += getCardIncome({ ...state, collection, misprints }, id);
  }
...
export function openPack(state, options = {}) {
  ...
  const applied = applyPulls(working, rolled.cards, context);
  working = applied.state;
  const highHit = rolled.cards.some((pull) => RARITIES[pull.rarity].order >= RARITIES.legendary.order);
  ...
  working = advanceBeat({
    ...
    pityLegendary: manual ? (highHit ? 0 : working.pityLegendary + 1) : working.pityLegendary,
  ...
  return {
    ...
    result: {
      ...
      source,
      context,
      incomeDelta: applied.incomeDelta,
      fusionEvents,
...
export function applyOfflineProgress(state, now = Date.now()) {
  ...
  const income = getBinderIncome(state) * elapsedSeconds;
  let next = addPassiveIncome(state, income);
  let ordered = 0;
  if (next.beat >= 2 && next.standingOrder?.enabled) {
    const product = PACK_PRODUCTS.find((candidate) => candidate.id === next.standingOrder.product);
    const price = getPackPrice(next, product?.id);
    if (product && product.unlockBeat <= next.beat && Number.isFinite(price)) {
      ordered = Math.max(0, Math.floor((next.coins - next.standingOrder.reserve) / price));
      ordered = Math.min(10_000, ordered);
      if (ordered > 0) {
        const cost = ordered * price;
        const sealed = cloneStock(next.sealed);
        sealed[next.standingOrder.setId][product.id] += ordered;
        next = {
          ...next,
          ...
          standingOrder: {
            ...next.standingOrder,
            purchased: next.standingOrder.purchased + ordered,
          },
        };
      }
    }
  ...
  return {
    state: next,
    report: income > 0 || ordered > 0
      ? { seconds: elapsedSeconds, coins: income, ordered }
      : null,
  };
}
