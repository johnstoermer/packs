...
export function sellDuplicatesDetailed(state, options = {}) {
  ...
  ctx.injectCards = Array.isArray(options.injectCards)
    ? options.injectCards.map((pull) => ({ ...pull }))
    : null;
...
// and removes the later slot so large generated packs never accumulate a tail
// of consumed cards and appended Fusion results.
export function resolveFusions(state, cards, options = {}) {
  ...
  const fuseInto = (rarity, left, right) => {
    ...
    if (immediate) {
      ...
      if (consumed !== null) nextCards.splice(consumed, 1);
