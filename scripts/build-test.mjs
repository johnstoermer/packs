...
await new Promise((done, reject) => {
  const child = spawn(process.execPath, [resolvePath("node_modules/next/dist/bin/next"), "build"], {
    stdio: "inherit",
    env: { ...process.env, PACKWORKS_TEST_BUILD: "1" },
  });
...
await cp("out", "dist-test", { recursive: true });
console.log("Built PACKWORKS test-channel bundle in dist-test/ (base /games/packs/test)");
